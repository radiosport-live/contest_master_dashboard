...
def app():
    ...
    command_run="select external_call as Last Run QSO, SEC_TO_TIME(timestampdiff(SECOND,`contest`.`qsos`.`timestamp`,current_timestamp())) as Time from qsos as q where q.mycall ='"+chosen_callsign+"' and IsRunQSO=1 ORDER BY timestamp DESC LIMIT 1;"
    command_sp="select external_call as 'Last S&P QSO', SEC_TO_TIME(timestampdiff(SECOND,`contest`.`qsos`.`timestamp`,current_timestamp())) as Time from qsos as q where q.mycall ='"+chosen_callsign+"' and IsRunQSO=0 ORDER BY timestamp DESC LIMIT 1;"
    ...
    command_radio1="SELECT r.OpCall, r.IsRunning, r.Freq, r.FunctionKeyCaption as Macro, r.IsTransmitting, r.FocusRadioNr from radio AS r WHERE r.station_call='"+chosen_callsign+"' and r.RadioNr=1 ORDER BY r.timestamp DESC Limit 1;" 
    command_radio2="SELECT r.OpCall, r.IsRunning, r.Freq, r.FunctionKeyCaption as Macro, r.IsTransmitting, r.FocusRadioNr from radio AS r WHERE r.station_call='"+chosen_callsign+"' and r.RadioNr=2 ORDER BY r.timestamp DESC Limit 1;"
